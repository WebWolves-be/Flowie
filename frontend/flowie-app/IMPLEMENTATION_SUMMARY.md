...
- [x] **DTO to Model Mapping**
    - ProjectDto → ProjectModel model (with computed progress)
    - TaskModel → Task model (1:1 mapping)
    - TaskTypeDto → TaskType model (1:1 mapping)
    - EmployeeDto → EmployeeModel model (for future use)
...
### 1. EmployeeModel Endpoint Missing ⚠️
...
The Angular frontend now communicates with the .NET backend through well-structured API services. All facade services
have been updated to use real HTTP calls instead of mock data. The implementation includes:
...
The only limitation is the missing employee endpoint in the backend, which has been handled gracefully with a fallback
to mock data.
